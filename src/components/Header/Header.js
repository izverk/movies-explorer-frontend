    ...
    <header className={`header app__wide-section${loggedIn ? ' header_background_no' : ''}`}>
      ...
      {!loggedIn ? (
        <div className='header__reg-links'>
          <Link to='/signup' className='app__link app__link_color_white header__signup-link'>Регистрация</Link>
          <Link to='/signin' className='app__link header__signin-link'>Войти</Link>
        </div>
      ) : (
